...
def student_get_courses():
    ...
    courses = db.session.query(Course.cno, Course.cname, Course.credit, Grade.grade, Course.term) \
        .join(Grade, Grade.cno == Course.cno) \
        .filter(Grade.sno == sno).all()
...
def admin_get_students():
    ...
    return jsonify({'success': True, 'students': [
        {'sno': s.sno, 'sname': s.sname, 'smajor': s.smajor, 'sclass': s.sclass, 'sex': s.sex, 'birthday': s.birthday}
        for s in students
    ]})
...
def admin_get_teachers():
    ...
    return jsonify({'success': True, 'teachers': [
        {'tno': t.tno, 'tname': t.tname, 'tdept': t.tdept}
        for t in teachers
    ]})
...
def admin_get_courses():
    ...
    return jsonify({'success': True, 'courses': [
        {'cno': c.cno, 'cname': c.cname, 'credit': c.credit, 'tno': c.tno, 'term': c.term}
        for c in courses
    ]})
...
def admin_get_grades():
    ...
    return jsonify({'success': True, 'grades': [
        {'id': g.id, 'sno': g.sno, 'cno': g.cno, 'term': g.term, 'grade': g.grade}
        for g in grades
    ]})
...
def teacher_update_info():
    ...
    # 更新教师信息（包括修改密码）
    updated_fields = {k: v for k, v in data.items() if k in ['tname', 'tdept', 'password']}
    ...
    if success:
        return jsonify({'success': True, 'message': '信息更新成功', 'teacher': teacher_data})
    else:
...
from database import Course  # Add this import
...
def teacher_get_grades():
    ...
    teacher_sno = session.get('user_id')
    teacher = db.get_teacher(teacher_sno)
    ...
    # 查询成绩并连接学生表以获取学生姓名
    grades = db.session.query(Grade, Student.sname).join(Student, Grade.sno == Student.sno).filter(Grade.cno == course.cno)
...
def student_update_info():
    ...
    # 更新学生信息（包括修改密码）
    updated_fields = {k: v for k, v in data.items() if
                      k in ['sname', 'smajor', 'sclass', 'sex', 'birthday', 'password']}
    if 'password' in updated_fields:
        updated_fields['password'] = db.hash_password(updated_fields['password'])
...
def student_get_grades():
    ...
    # 查询该学生的成绩，并连接课程表
    grades_query = db.session.query(Grade, Course).join(Course, Grade.cno == Course.cno).filter(Grade.sno == sno)
